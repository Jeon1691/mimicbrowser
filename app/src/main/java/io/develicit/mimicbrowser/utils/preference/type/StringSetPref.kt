...
internal class StringSetPref(private val key: String) : ReadOnlyProperty<PrefModel, MutableSet<String>> {
    ...
    override operator fun getValue(thisRef: PrefModel, property: KProperty<*>): MutableSet<String> {
        if (tempSet == null || update.get() < thisRef.lastUpdate()) {
            thisRef.pref.getStringSet(key, null).takeIf { it.isNotEmpty() }?.let {
                tempSet = PrefSet(thisRef, it, key)
